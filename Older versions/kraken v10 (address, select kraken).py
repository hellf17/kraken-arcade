...
class StartMenuView(arcade.View):
    def __init__(self, window):
        ...
        self.user_address = PasteETHAddress()
        self.last_five_digits = self.user_address.address[-5:]
        self.token_id = None
    ...
    def setup(self):
        ...
        self.user_address = arcade.gui.UIInputText(
            ...
            font_name="Minecraft",
            font_size=25,
            text=self.user_address.address,
    ...
    def on_key_press(self, key, modifiers):
        if key == arcade.key.N:
            ...
            game_view = KrakenGame(SCREEN_WIDTH, SCREEN_HEIGHT, "Kraken Game", user_address=self.user_address.text, token_id=int(self.token_id.text))
            game_view.setup()
...
class Kraken(arcade.AnimatedTimeBasedSprite):
    def __init__(self, user_address=None, token_id=None):
        ...
        self.xp = 0
        self.active_buffs = []
        self.last_five_digits = str(user_address)[-5:]

        if user_address and token_id:
            self.textures = []
            for i in range(0, 10):
                texture = arcade.load_texture(f".\\{self.last_five_digits}\\kraken_{self.token_id}\\kraken_frame_{i}.png")
                self.textures.append(texture)
        else:
            self.textures = []
            for i in range(0, 10):
...
class KrakenGame(arcade.View):
    ...
    def setup(self):
        ...
        self.player = Kraken(self.user_address, int(self.token_id))
