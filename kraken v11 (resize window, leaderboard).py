...
def leaderboard(user_address, token_id, score):
    # Initialize Moralis with your application ID and server URL
    moralis.initialize("YOUR_MORALIS_APP_ID")
    moralis.server_url = "YOUR_MORALIS_SERVER_URL"

    # Initialize web3 provider from Moralis
    web3_provider = moralis.Web3Provider("ropsten")  # Use the Ethereum testnet where your contract is deployed

    # Leaderboard contract address
    contract_address = '0x630B181874BA2e16b03FB32eb67D086c00891418'
    with open('leaderboard_abi.json', 'r') as file:
        contract_abi = json.load(file)

    # Creates the contract object
    contract = w3.eth.contract(address=contract_address, abi=contract_abi)

    # Register a new score using the secret key (called from your game)
    def register_score(token_id, score, secret_key):
        try:
            # Connect to the blockchain using the game address's private key
            web3 = Web3(web3_provider.make_requests())
            ...
            # Send the transaction with the provided secret key
            tx_hash = contract.functions.registerScore(token_id, score, secret_key).transact(
                {"from": web3.eth.account.privateKeyToAccount(game_private_key).address}
            )
...
def validate_user_address(user_address):
    if user_address is not None:
        ...
        return re.match(pattern, str(user_address.text))

def validate_token_id(token_id):
    if token_id is not None:
        return str(token_id).isdigit() and 0 <= int(token_id) <= 2999

class PasteETHAddress:
    def __init__(self):
...
class StartMenuView(arcade.View):
    def __init__(self, window):
        ...
        self.user_address = PasteETHAddress().address
        self.last_five_digits = self.user_address[-5:]
        self.token_id = None
    ...
    def setup(self):
        ...
        self.user_address = arcade.gui.UIInputText(
            ...
            font_name="Minecraft",
            font_size=25,
            text=self.user_address,
            text_color=arcade.color.WHITE,
            style=None,
        )

        self.token_id = arcade.gui.UIInputText(
        ...
        self.ui_root = arcade.gui.UIBorder(
            border_width=3,
            border_color=(255, 255, 255, 255),
            child=self.user_address,
        )

        self.ui_root1 = arcade.gui.UIBorder(
            border_width=3,
            border_color=(255, 255, 255, 255),
            child=self.token_id,
        )
        self.manager.add(self.ui_root)
        self.manager.add(self.ui_root1)
    ...
    def on_draw(self):
        ...
        arcade.draw_text("Start Menu", SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2,
                         arcade.color.WHITE, font_size=70, font_name="Minecraft", anchor_x="center")
        arcade.draw_text("Press 'N' to start a new game - It might take some seconds to load your Kraken", SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 - 50,
                         arcade.color.WHITE, font_size=30, font_name="Minecraft", anchor_x="center")
        arcade.draw_text("Press 'O' for options", SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 - 90,
                         arcade.color.WHITE, font_size=30, font_name="Minecraft", anchor_x="center")
        arcade.draw_text("Press 'ESC' to exit", SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2 - 130,
                         arcade.color.WHITE, font_size=30, font_name="Minecraft", anchor_x="center")
    ...
    def on_key_press(self, key, modifiers):
        if key == arcade.key.N:
            ...
            if validate_user_address(self.user_address) and validate_token_id(self.token_id.text):
               token_dir = f"./{self.last_five_digits}/kraken_{int(self.token_id.text)}"
               if not os.path.exists(token_dir):
                   metadata_extractor.get_all_token_metadata(self.user_address.text, int(self.token_id.text))

            game_view = KrakenGame(SCREEN_WIDTH, SCREEN_HEIGHT, "Kraken Game", user_address=self.user_address, token_id=self.token_id)
            game_view.setup()
            self.window.show_view(game_view)

        elif key == arcade.key.O:
            options_view = OptionsView(self.window, self)
...
class Kraken(arcade.AnimatedTimeBasedSprite):
    def __init__(self, user_address=None, token_id=None):
        super().__init__()
        ...
        if user_address and token_id:
            self.last_five_digits = self.user_address.text[-5:]
            self.textures = []
            for i in range(0, 10):
                texture = arcade.load_texture(f".\\{self.last_five_digits}\\kraken_{self.token_id.text}\\kraken_frame_{i}.png")
                self.textures.append(texture)
        else:
            self.textures = []
            for i in range(0, 10):
...
class KrakenGame(arcade.View):
    def __init__(self, width, height, title, user_address=None, token_id=None):
        super().__init__()
        self.user_address = user_address
        self.token_id = token_id
    ...
    def setup(self):
        ...
        if validate_user_address(self.user_address) and validate_token_id(self.token_id.text):
            self.player = Kraken(self.user_address, self.token_id)
        else:
            self.player = Kraken()
