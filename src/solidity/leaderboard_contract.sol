...
contract Leaderboard {
    ...
    modifier onlyGame() {
        require(msg.sender == owner || msg.sender == gameContractAddress, "Only the game can register scores");
        _;
    }

    address public gameContractAddress; //Need to set this to the game contract address
    ...
    function registerScore(uint256 token_id, uint256 score, uint256 timeSurvived, uint256 enemiesKilled) external onlyGame {
        latestScoreId++;
        scores[latestScoreId] = Score(msg.sender, token_id, score, timeSurvived, enemiesKilled);
        updateLeaderboard();
    }
